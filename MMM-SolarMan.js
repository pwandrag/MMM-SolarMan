...
Module.register("MMM-SolarMan",{
	...
	drawSummaryTable:  function(data){
		...
		row.innerHTML = `<td class='small regular bright'>System</td><td class='small light bright'>${data.instantaneous.status}</td><td class='small regular bright'>Grid<span class='small light bright' style='float:right'">${data.instantaneous.gridStatus}</span></td><td><span class='normal xsmall' style='float:right'>Token:${new Date(data.instantaneous.tokenExpiration).toLocaleDateString('en-us',[{day:'numeric'},{month:'short'},{year:'none'}])}</span></td>`;
