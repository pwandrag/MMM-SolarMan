...
Module.register("MMM-SolarMan",{
	...
	drawSummaryTable:  function(data){
		...
		row.innerHTML = `<td class='small regular bright'>System</td><td class='small light bright'>${data.instantaneous.status}</td><td colspan=2><span class='normal xsmall' style='float:right'>Token:${new Date(data.instantaneous.tokenExpiration).toLocaleDateString('en-us',[{day:'numeric'},{month:'short'},{year:'none'}])}</span></td>`;
		table.appendChild(row);
		row = document.createElement("tr");
		row.innerHTML = `<td class='small regular bright'>Load</td><td class='small light bright'>${data.instantaneous.load} W</td><td class='normal small light'>${data.today.loadToday} KW</td><td class='normal xsmall light'>${data.yesterday.loadPrevDay} KW</td>`;
		table.appendChild(row);
		row = document.createElement("tr");
		row.innerHTML = `<td class='small regular bright'>Generating</td><td class='small light bright'>${data.instantaneous.generating} W</td><td class='normal small light'>${data.today.generationPowerToday} KW</td><td class='normal xsmall light'>${data.yesterday.generationPowerPrevDay} KW</td>`;
		table.appendChild(row);
		row = document.createElement("tr");
		row.innerHTML = `<td class='small regular bright'>Battery</td><td class='small light bright'>${data.instantaneous.battery} W</td><td class='normal small light'>${data.today.batteryPowerToday} KW</td><td class='normal xsmall light'>${data.yesterday.batteryPowerPrevDay} KW</td>`;
		table.appendChild(row);
		row = document.createElement("tr");
		row.innerHTML = `<td class='small regular bright'>Grid</td><td class='small light bright'>${data.instantaneous.grid} W</td><td class='normal small light'>${data.today.gridPowerToday} KW</td><td class='normal xsmall light'>${data.yesterday.gridPowerPrevDay} KW</td>`;
		table.appendChild(row);
		row = document.createElement("tr");
